...
%(1) Scores of 17 SDGs are in 17 sheets of one excel file
%(2) In each sheet, there is a 285 (cities) * 17 (years) Matrix since there are 285 Chinese cities and period is from 2005 to 2016
%(3) In each sheet, B column show city name and 2ed row shows period
%% Step 1: Input basic parameters
clear all;
city_n = 285;% city number
SDG_n = 17;% SDG number
threshold = 0;% Range [0,100]; Business as usual=0; slight policy=10; moderate policy=20; aggressive policy=30
...
num=1;
for sheet_num = 10:26  % 17 sheets correspond to 17 SDGs
    str =['Reading completed ' num2str(num/17*100) ' %'];
    disp(str)
    M = readmatrix('Supplementary tables 20220220.xlsx','UseExcel',1,'Sheet',sheet_num,'Range' ,[3 3 287 14]);% 'sheet':10; 'Range':numeric inputs [r1 c1 r2 c2]   
    year_2016(:,num) = M(:,12);
    for i = 1:length(M)
        tmp = (M(i,12)/M(i,1))^(1/11) -1;
        if tmp < 0
            tmp = 0;
        elseif tmp > 1
            tmp =1;
...
xlswrite('SDG_predict_year_0.xls',SDG_predict_year(:,:),'Sheet1') % Export the scores of the SDGs resutls